...
async function previewSubmission(sid,number,kind){
  ...
  if(remoteUrl){
    try{
      ...
      const typedBlob=new Blob([rawBlob],{type:kind==='pdf'?'application/pdf':'text/html'});
    ...
    }catch(e){toast('Unable to load from GitHub. '+e.message)}
    return;
  }
  if(!meta.key){toast('This file has not been pushed to GitHub yet.');return}
  try{
    const blob=await getSubmissionBlob(meta.key);
    if(!blob){toast('Stored file is missing. Please upload it again.');return}
    if(activePreviewUrl)URL.revokeObjectURL(activePreviewUrl);
    activePreviewUrl=URL.createObjectURL(blob);
    frame.src=activePreviewUrl;
    openDlg('#submission-preview');
  }catch(e){toast('Unable to open preview. '+e.message)}
}
...
function closeSubmissionPreview(){closeDlg('#submission-preview');$('#preview-frame').src='about:blank';if(activePreviewUrl){URL.revokeObjectURL(activePreviewUrl);activePreviewUrl=null}}
...
function enterStudent(id,evt){activeStudent=id;state.selectedStudent=id;saveState();const x=evt?.clientX||innerWidth/2,y=evt?.clientY||innerHeight/2;startPortalCameraTransition(id,x,y,()=>{renderStudent();showView('student')})}
...
function resubmission(form){const sid=form.dataset.sid,n=Number(form.dataset.mission),note=$('#mark-feedback').value.trim()||'Please revise and submit again.';state.students[sid]=requestResubmission(state.students[sid],n,note);saveState();renderLecturer('queue');toast('Resubmission requested.')}
