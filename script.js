...
function initSunEasterEgg(){
  ...
  sun.addEventListener('click',()=>{
    ...
    if(sunClickTimes.length>=3){
      sunClickTimes=[];
      const scene=sun.closest('.corridor-scene');
...
function initHeroParallax(){
  const scene=document.querySelector('.corridor-scene'); if(!scene)return;
  if(reduceMotion()||!window.matchMedia?.('(hover:hover) and (pointer:fine)').matches)return;
  const band=scene.closest('.corridor-intro-section'); if(!band)return;
...
renderHome();bind();showView('home');initCinematicScroll();initBootSplash();initSunEasterEgg();initHeroParallax();
