...
function createMission(student,title,description){
  ...
  s.missions.push({number,title:String(title||'').slice(0,120),description:String(description||'').slice(0,600),status:'OPEN',pdfFile:null,htmlFile:null,assets:[],versions:[],createdAt:new Date().toISOString(),submittedAt:null,gradedAt:null,mark:null,maxMark:100,feedback:'',resubmissionNote:'',due:''});
  return s;
}
...
function setSubmissionPackage(student,number,pkg){return updateMission(student,number,m=>{m.pdfFile=pkg?.pdfFile||m.pdfFile||null;m.htmlFile=pkg?.htmlFile||m.htmlFile||null;if(pkg?.assets)m.assets=pkg.assets;else if(!m.assets)m.assets=[];return m})}
...
/* ===== GitHub push proxy config =====
   No token is stored in the browser at all. Every push goes through a
   Cloudflare Worker (see cloudflare-worker.js) which holds the token as a
   server-side secret. Fill in WORKER_URL below after deploying the Worker
   (see the deployment instructions at the top of cloudflare-worker.js). */
const WORKER_URL='https://jobverse-proxy.aim77495.workers.dev';
const GH_DEFAULTS={owner:'aiman-man',repo:'Group-Project-20-Percent',branch:'main',prefix:'submissions'};
function getGithubConfig(){return {...GH_DEFAULTS}}
function githubConfigured(){return !!WORKER_URL}
...
async function githubPutFile(path,file,message){
  if(!WORKER_URL)throw new Error('JOBVERSE is not connected to a push proxy yet.');
  const content=await fileToBase64(file);
  const res=await fetch(`${WORKER_URL}/push`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path,content,message})});
...
let state=loadState(); let activeStudent='s1', activeMission=1, pendingPackage={pdfFile:null,htmlFile:null,assets:[]}, pendingBlobs={pdf:null,html:null,assets:[]}; let soundOn=false; let activePreviewUrl=null;
...
async function persistPendingPackage(){
  ...
  pendingPackage={pdfFile:pdfMeta,htmlFile:htmlMeta,assets:pendingPackage.assets||[]};
  ...
  saveState();pendingBlobs.pdf=null;pendingBlobs.html=null;
...
async function syncFromGithub(opts){
  ...
  if(!githubConfigured()){if(!silent)console.warn('Push proxy not configured.');return}
  ...
  try{
    ...
    for(const sid of ['s1','s2']){
      ...
      for(const g of Object.values(groups)){
        ...
        if(idx<0){
          s.missions.push({
            ...
            createdAt:metaPayload?.submittedAt||new Date().toISOString(),
          ...
          s.missions[idx]={...existing,
            ...
            createdAt:existing.createdAt||metaPayload.submittedAt||new Date().toISOString(),
      ...
      s.missions.forEach(m=>{if(!m.createdAt){m.createdAt=m.submittedAt||new Date().toISOString();updated++}});
    }
...
function openMission(number){
  ...
  pendingPackage={pdfFile:m.pdfFile?clone(m.pdfFile):null,htmlFile:m.htmlFile?clone(m.htmlFile):null,assets:m.assets?clone(m.assets):[]};
  pendingBlobs={pdf:null,html:null,assets:[]};
...
async function submitCurrentMission(bx,by){
  ...
  try{
    if(!githubConfigured()){toast('Push service is not available right now. Please try again shortly.');return}
    ...
    let assetsMeta=m0.assets||[];
    if((pendingBlobs.assets||[]).length){
      const pushed=[];
      for(const af of pendingBlobs.assets){
        const r=await githubPutFile(submissionGithubAssetPath(s0,m0,af),af,`Add linked file ${af.name} for ${m0.title}`);
        pushed.push({name:af.name,rawUrl:r.rawUrl,liveUrl:r.liveUrl});
      }
      const pushedNames=new Set(pushed.map(p=>p.name));
      assetsMeta=[...assetsMeta.filter(a=>!pushedNames.has(a.name)),...pushed];
      pendingBlobs.assets=[];
    }
    state.students[activeStudent]=setSubmissionPackage(state.students[activeStudent],activeMission,{pdfFile:pdfMeta,htmlFile:htmlMeta,assets:assetsMeta});
...
function attachDropZoneMulti(labelEl,inputEl,onFiles){
  if(!labelEl||!inputEl)return;
  ['dragenter','dragover'].forEach(ev=>labelEl.addEventListener(ev,e=>{e.preventDefault();e.stopPropagation();labelEl.classList.add('dragover')}));
  ['dragleave','dragend','drop'].forEach(ev=>labelEl.addEventListener(ev,e=>{e.preventDefault();e.stopPropagation();labelEl.classList.remove('dragover')}));
  labelEl.addEventListener('drop',e=>{const fs=e.dataTransfer?.files;if(fs&&fs.length)onFiles(fs)});
}
function submissionGithubAssetPath(student,mission,file){const folder=submissionGithubFolder(student,mission);return `${folder}/${file.name}`.replace(/\/+/g,'/')}
function acceptAssetFiles(fileList){
  const files=Array.from(fileList||[]);
  if(!files.length)return;
  pendingBlobs.assets=(pendingBlobs.assets||[]).concat(files);
  renderDialogPackage();
  beep(540,.05);
}
...
function bind(){
  ...
  $('#brand-home').onclick=goHomeWarp;$$('[data-route-home]').forEach(b=>b.onclick=goHomeWarp);$$('[data-home-anchor]').forEach(b=>b.onclick=()=>{showView('home');setTimeout(()=>document.getElementById(b.dataset.homeAnchor)?.scrollIntoView({behavior:'smooth'}),20)});$$('[data-enter-student]').forEach(b=>b.onclick=e=>enterStudent(b.dataset.enterStudent,e));const goAbout=()=>{showView('home');setTimeout(()=>document.getElementById('about')?.scrollIntoView({behavior:'smooth'}),20)};$('#nav-about').onclick=goAbout;$('#sound-toggle').onclick=()=>{soundOn=!soundOn;$('#sound-toggle').textContent=soundOn?'SOUND ON':'SOUND OFF';$('#sound-toggle').setAttribute('aria-pressed',String(soundOn));beep(540,.07);if(soundOn){startAmbientHum();const bg=document.getElementById('bg-music');if(bg){bg.volume=1;bg.currentTime=0;bg.play().catch(()=>{});clearTimeout(window._bgMusicLimit);window._bgMusicLimit=setTimeout(()=>{bg.pause()},BG_MUSIC_LIMIT_SECONDS*1000)}}else{stopAmbientHum();const bg=document.getElementById('bg-music');if(bg)bg.pause();clearTimeout(window._bgMusicLimit)}};
  $('#nav-lecturer').onclick=openLecturerLogin;$('#lecturer-login-close').onclick=()=>closeDlg('#lecturer-login');$('#lecturer-login-submit').onclick=submitLecturerLogin;$('#lecturer-passcode').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();e.stopPropagation();submitLecturerLogin()}});$('#new-submission-btn').onclick=openNewSubmission;
  ...
  $$('.student-nav [data-student-section], [data-student-section]').forEach(b=>b.onclick=()=>showStudentSection(b.dataset.studentSection));$('#student-search').oninput=renderMissionList;$('#student-filter').onchange=renderMissionList;$('#mission-close').onclick=()=>closeDlg('#mission-dialog');$('#mission-dialog').addEventListener('close',cleanupEmptyDraft);$('#preview-close').onclick=closeSubmissionPreview;$('#submission-preview').addEventListener('close',closeSubmissionPreview);$('#mission-pdf-file').onchange=e=>{const f=e.target.files?.[0];acceptPdfFile(f);if(!f)e.target.value=''};$('#mission-html-file').onchange=e=>{const f=e.target.files?.[0];acceptHtmlFile(f);if(!f)e.target.value=''};$('#mission-assets-files').onchange=e=>{acceptAssetFiles(e.target.files);e.target.value=''};attachDropZone($('#pdf-slot'),$('#mission-pdf-file'),acceptPdfFile);attachDropZone($('#html-slot'),$('#mission-html-file'),acceptHtmlFile);attachDropZoneMulti($('#assets-slot'),$('#mission-assets-files'),acceptAssetFiles);$('#mission-save').onclick=savePendingPackage;$('#mission-delete').onclick=deleteCurrentMission;$('#mission-submit').onclick=e=>{const r=e.currentTarget.getBoundingClientRect();submitCurrentMission(r.left+r.width/2,r.top+r.height/2)};
...
renderHome();bind();showView('home');initCinematicScroll();initBootSplash();initSunEasterEgg();initHeroParallax();initPlanet3D();autoSyncIfDue();
